...
# Last Modified: 2025/08/02 17:25:27
#******************************************#
...
#########################################
# SEGLIST =
def _makeSEGLIST(fecs):
    return { ### 'name' is only used for overriting segname
        ii+1: {
            'name': 'K1:DAQ_IPC_ERROR_{0}:1'.format(fecs[fec]),
            'function': IPC._make_ipc_glitch_flag,
            'option': [False],
            'channel': {'default': ['K1:FEC-{0}_TIME_DIAG'.format(fec)]}
        }
        for ii, fec in enumerate(fecs.keys())
    }
...
if __name__ == '__main__':
    ...
    for ii_gps in gpsrange:
        ...
        SEGLIST = _makeSEGLIST(FECs[term])
